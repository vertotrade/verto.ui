...
import { Modal, Text, Flex, BalanceInput, Box, Button, LogoRoundIcon, useToast } from '@verto/uikit'
import { useAccount } from 'wagmi'
import { useTranslation } from '@verto/localization'
import { formatNumber, getBalanceAmount, getBalanceNumber } from '@verto/utils/formatBalance'
...
const PlaceBidModal: React.FC<React.PropsWithChildren<PlaceBidModalProps>> = ({
  ...
  useEffect(() => {
    if (userNotEnoughCake) {
      setErrorText(t('Insufficient VERTO balance'))
    } else if (!isMoreThanInitialBidAmount && isFirstBid) {
      setErrorText(t('First bid must be %initialBidAmount% VERTO or more.', { initialBidAmount }))
    ...
    <StyledModal title={t('Place a Bid')} onDismiss={onDismiss} headerBackground={theme.colors.backgroundAlt}>
      <ExistingInfo>
        <Flex justifyContent="space-between">
          ...
          <Text>{t('%num% VERTO', { num: getBalanceNumber(amount).toLocaleString() })}</Text>
        </Flex>
        <Flex justifyContent="space-between">
      ...
      <InnerContent>
        <Flex justifyContent="space-between" alignItems="center" pb="8px">
          ...
          <Flex>
            ...
            <Text bold>VERTO</Text>
          </Flex>
        </Flex>
        {isFirstBid && (
          <Text pb="8px" small>
            {t('First bid must be %initialBidAmount% VERTO or more.', { initialBidAmount })}
          </Text>
        )}
        <BalanceInput
