...
import { Button, AutoRenewIcon, Box, Flex, Message, MessageText, Text } from '@verto/uikit'
import _noop from 'lodash/noop'
import { useTranslation } from '@verto/localization'
import isUndefinedOrNull from '@verto/utils/isUndefinedOrNull'
import { MAX_LOCK_DURATION } from 'config/constants/pools'
import BigNumber from 'bignumber.js'
import { getBalanceAmount, getDecimalAmount } from '@verto/utils/formatBalance'
      ...
      {!needsApprove && cakeNeeded ? (
        hasEnoughBalanceToExtend ? (
          <Text fontSize="12px" mt="24px">
            {t('0.0001 VERTO will be spent to extend')}
          </Text>
        ) : (
          <Message variant="warning" mt="24px">
            <MessageText maxWidth="200px">{t('0.0001 VERTO required for enabling extension')}</MessageText>
          </Message>
        )
      ) : null}
