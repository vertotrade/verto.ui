...
import { Flex, Text, TooltipText, useTooltip, Box, Link, BalanceWithLoading } from '@verto/uikit'
...
const IfoCakeRow: React.FC<React.PropsWithChildren> = () => {
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    <Box>
      <Text>
        {t(
          'The number of iVERTO equals the locked staking amount if the staking duration is longer than %weeks% weeks. If the staking duration is less than %weeks% weeks, it will linearly decrease based on the staking duration.',
          {
            weeks: weeksDisplay,
          },
        )}
      </Text>
      <InlineLink external href="https://docs.vertotrade.com/products/ifo-initial-farm-offering/iverto">
        {t('Learn more about iVERTO')}
      </InlineLink>
    </Box>,
    {
