import { Box, Button, Flex, Text, useModal } from '@verto/uikit'
...
import { CollectionLink, PriceBox } from '../shared/styles'
...
const NFTHeaderDetails: React.FC<React.PropsWithChildren<MainNFTCardProps>> = ({
  nft,
  isOwnNft,
  nftIsProfilePic,
  ...
  return (
    <Box mb="40px" style={{ alignSelf: 'flex-start', top: '34px' }} position={['relative', 'relative', 'sticky']}>
      <CollectionLink to={`${nftsBaseUrl}/collections/${nft.collectionAddress}`}>{nft?.collectionName}</CollectionLink>
      ...
      <PriceBox>
        ...
        {nftIsProfilePic && (
          <Text color="failure">
            {t('This NFT is your profile picture, you must change it to some other NFT if you want to sell this one.')}
          </Text>
        )}
...
export default NFTHeaderDetails
