...
import { AutoRenewIcon, NextLinkFromReactRouter, Button } from '@verto/uikit'
    ...
    switch (boosterState) {
      ...
      case YieldBoosterState.NO_LOCKED:
        return (
          <ActionButton
            title={`${t('Up to')} ${boostMultiplierDisplay}x`}
            description={t('Lock VERTO to activate yield booster')}
      ...
      case YieldBoosterState.LOCKED_END:
        return (
          <ActionButton
            title={`${t('Up to')} ${boostMultiplierDisplay}x`}
            description={t('Lock VERTO is ended. Re-lock VERTO to activate yield booster')}
            style={{ whiteSpace: 'nowrap' }}
            button={
              <NextLinkFromReactRouter to="/pools">
                <Button> {t('Go to Pool')}</Button>
